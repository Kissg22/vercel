...
async function recalcCustomer(shopName, token, customerId, changedOrderId = null) {
  ...
  // 1) REST: Paginated fetch—partial if changedOrderId provided
  let orders = [];
  ...
  try {
    if (changedOrderId) {
      // Fetch pages until changedOrderId is found
      do {
        const params = new URLSearchParams({ status: 'any', customer_id: customerId, limit: '250' });
        ...
        console.log(`✅ Retrieved ${pageOrders.length} orders`);
        orders.push(...pageOrders);

        // Check if changedOrderId is in this batch
        if (orders.some(o => String(o.id) === String(changedOrderId))) {
          foundChanged = true;
          console.log(`✅ Found changedOrderId ${changedOrderId}`);
          break;
        }
      ...
      // If not found, fall back to full fetch
      if (!foundChanged) {
        console.warn(`⚠️ changedOrderId ${changedOrderId} not found in initial pages—fetching all orders`);
        orders = [];
        nextPageInfo = null;
        do {
          const params = new URLSearchParams({ status: 'any', customer_id: customerId, limit: '250' });
      ...
      // No changedOrderId: fetch all orders
      do {
        const params = new URLSearchParams({ status: 'any', customer_id: customerId, limit: '250' });
        if (nextPageInfo) params.set('page_info', nextPageInfo);
  ...
  // 2) Seed and determine startIndex
  let cumSpend  = 0;
  let cumShares = 0;
  let startIndex = 0;
  ...
  if (changedOrderId) {
    ...
    if (idx > 0) {
      ...
      console.log(`🔍 Seeding from order ${prev.name} (${prev.id})`);
      try {
        const metaResp = await fetch(graphqlEndpoint, {
          ...
          body: JSON.stringify({
            ...
            variables: { id: `gid://shopify/Order/${prev.id}` }
          })
        });
        ...
        console.error('❌ Error fetching seed metafield:', e);
      }
      startIndex = idx;
      console.log(`🔄 Recalculating from index ${startIndex}`);
    } else {
      console.log(`⚠️ Order ${changedOrderId} is first or not found, starting from 0`);
      startIndex = 0;
    }
  }

  // 3) Recalculate from startIndex onward
  for (let i = startIndex; i < orders.length; i++) {
    ...
    if (o.cancelled_at) {
      console.log(`↩️  Order ${name} cancelled → effective=0`);
    } else {
      ...
      try {
        ...
        for (const r of refunds) {
          for (const li of r.refund_line_items || []) {
            const amt = parseFloat(li.subtotal_set?.presentment_money?.amount ?? li.subtotal ?? 0);
            refunded += amt;
          }
        }
      } catch (e) {
    ...
    console.log(`🔢 After order ${name}: cumSpend=${cumSpend.toFixed(2)}, shares this order=${orderShares}, cumShares=${cumShares}, remainder=${remainder.toFixed(2)}`);
    ...
    try {
      ...
      const resp = await fetch(graphqlEndpoint, { method: 'POST', headers: { 'X-Shopify-Access-Token': token, 'Content-Type': 'application/json' }, body: JSON.stringify({ query: mutation, variables }) });
  ...
  try {
    ...
    const resp = await fetch(graphqlEndpoint, { method: 'POST', headers: { 'X-Shopify-Access-Token': token, 'Content-Type': 'application/json' }, body: JSON.stringify({ query: custMutation, variables: custVariables }) });
    const j = await resp.json();
